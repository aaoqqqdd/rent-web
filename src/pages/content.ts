...
function renderOrderCard(order: OrderView): string {
  ...
  const contractBlock = order.contract
    ...
    : ''
  const contractAction = order.contract?.status === 'pending_sign' && order.contract.url
    ? `<a class="btn btn-primary order-contract-action" href="${esc(order.contract.url)}" target="_blank" rel="noopener">签署合同</a>`
    : ''
...
(() => {
  ...
  function renderOrder(order, temporaryPassword) {
    ...
    var actions = document.createElement('div'); actions.className = 'order-record-head-actions'; var status = document.createElement('span'); status.className = 'order-status'; status.textContent = order.statusLabel; actions.appendChild(status); if (order.contract && order.contract.status === 'pending_sign' && order.contract.url) { var signLink = document.createElement('a'); signLink.className = 'btn btn-primary order-contract-action'; signLink.href = order.contract.url; signLink.target = '_blank'; signLink.rel = 'noopener'; signLink.textContent = '签署合同'; actions.appendChild(signLink); } head.append(headCopy, actions); article.appendChild(head);
