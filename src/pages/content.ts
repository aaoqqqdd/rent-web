...
<section class="section" id="delivery">
  <div class="wrap delivery-grid">
    <div><div class="kicker">取还范围</div><h2>墨尔本本地交付</h2><p>${esc(config.deliveryNote)} 当前可配送区域包括：${esc(deliveryAreas)}。</p><p>填写完整地址后会先获取预估配送费；具体配送时段、地址是否可服务和最终费用会在订单审核时确认。其他墨尔本郊区请选择到店自取。</p><p>当前可选自取 / 归还地点：</p>${pickupLocations}</div>
...
function renderOrderCard(order: OrderView): string {
  ...
  const contractBlock = shouldShowContract
    ? order.contract
      ...
      : `<section class="order-record-section"><p class="section-code">CONTRACT</p><h3>合同</h3><p class="section-note contract-empty-note">订单已审核通过，合同签署链接正在生成，请稍后刷新订单。</p></section>`
    : ''
  const contractAction = order.contract?.url
    ...
    : ''
  const deliveryBlock = order.deliveryBookings.length
    ? `<section class="order-record-section"><p class="section-code">DELIVERY</p><h3>配送状态</h3>${order.deliveryBookings.map(booking => `<div class="delivery-status-row"><div><strong>${booking.direction === 'return' ? '回收' : '派送'}：${esc(booking.statusLabel)}</strong><p class="section-note">${esc(booking.statusDescription)}</p>${booking.providerReference ? `<small class="section-note">配送编号：${esc(booking.providerReference)}</small>` : ''}${booking.locked ? '<small class="delivery-lock-note">该状态下不能更新或取消配送订单。</small>' : ''}</div>${booking.trackingUrl && /^https?:\/\//i.test(booking.trackingUrl) ? `<a class="link-button" href="${esc(booking.trackingUrl)}" target="_blank" rel="noopener noreferrer">查看追踪链接</a>` : ''}</div>`).join('')}</section>`
    : ''
  return `<article class="order-record">
    ...
    <div class="order-record-grid">
      <section class="order-record-section"><p class="section-code">ORDER DETAIL</p><h3>订单详情</h3><dl class="lookup-data-list">${orderDetail('订单编号', order.orderNo)}${orderDetail('下单时间', order.createdAt)}${orderDetail('租期', `${lookupDate(order.startDate)} 至 ${lookupDate(order.endDate)}（${order.rentalPeriod || 0} 天）`)}${orderDetail('取还方式', order.deliveryMethod === 'Delivery' ? '送货上门' : '到店自取')}${orderDetail('取货地点', order.pickupLocation)}${orderDetail('归还地点', order.returnLocation)}${orderDetail('订单备注', order.rentalNote)}</dl></section>
      ...
      ${deliveryBlock}
      <div class="order-payment-contract-row">${paymentBlock}${contractBlock}</div>
      ${credentialBlock}
    </div>
  </article>`
}
...
(() => {
  ...
  function renderOrder(order, temporaryPassword) {
    ...
    var detail = section('订单详情', 'ORDER DETAIL'); var detailList = document.createElement('dl'); detailList.className = 'lookup-data-list'; addDetail(detailList, '订单编号', order.orderNo); addDetail(detailList, '下单时间', order.createdAt); addDetail(detailList, '租期', formatDate(order.startDate) + ' 至 ' + formatDate(order.endDate) + '（' + (order.rentalPeriod || 0) + ' 天）'); addDetail(detailList, '取还方式', order.deliveryMethod === 'Delivery' ? '送货上门' : '到店自取'); addDetail(detailList, '取货地点', order.pickupLocation); addDetail(detailList, '归还地点', order.returnLocation); addDetail(detailList, '订单备注', order.rentalNote); detail.appendChild(detailList); grid.appendChild(detail);
    ...
    if (order.deliveryBookings && order.deliveryBookings.length) { var delivery = section('配送状态', 'DELIVERY'); order.deliveryBookings.forEach(function (booking) { var row = document.createElement('div'); row.className = 'delivery-status-row'; var copy = document.createElement('div'); var title = document.createElement('strong'); title.textContent = (booking.direction === 'return' ? '回收：' : '派送：') + booking.statusLabel; var note = document.createElement('p'); note.className = 'section-note'; note.textContent = booking.statusDescription; copy.append(title, note); if (booking.providerReference) { var ref = document.createElement('small'); ref.className = 'section-note'; ref.textContent = '配送编号：' + booking.providerReference; copy.appendChild(ref); } if (booking.locked) { var lock = document.createElement('small'); lock.className = 'delivery-lock-note'; lock.textContent = '该状态下不能更新或取消配送订单。'; copy.appendChild(lock); } row.appendChild(copy); if (booking.trackingUrl && /^https?:\/\//i.test(booking.trackingUrl)) { var link = document.createElement('a'); link.className = 'link-button'; link.href = booking.trackingUrl; link.target = '_blank'; link.rel = 'noopener noreferrer'; link.textContent = '查看追踪链接'; row.appendChild(link); } delivery.appendChild(row); }); grid.appendChild(delivery); }
    var payment = section('费用与付款', 'PAYMENT'); var paymentList = document.createElement('dl'); paymentList.className = 'lookup-data-list'; var rentalAmount = Math.max(0, Number(order.totalAmount || 0) - Number(order.depositAmount || 0) - Number(order.deliveryFee || 0)); addDetail(paymentList, '租赁费用', 'AUD$' + rentalAmount.toFixed(2)); addDetail(paymentList, '配送费', 'AUD$' + Number(order.deliveryFee || 0).toFixed(2)); addDetail(paymentList, '押金', 'AUD$' + Number(order.depositAmount || 0).toFixed(2)); addDetail(paymentList, '订单合计', 'AUD$' + Number(order.totalAmount || 0).toFixed(2)); if (Number(order.amountDue || 0) > 0) addDetail(paymentList, '待支付', 'AUD$' + Number(order.amountDue).toFixed(2)); addDetail(paymentList, '付款状态', order.paymentStatus); addDetail(paymentList, '押金状态', order.depositStatus); payment.appendChild(paymentList);
    var paymentContract = document.createElement('div'); paymentContract.className = 'order-payment-contract-row'; paymentContract.appendChild(payment);
    if (order.contract) { var contract = section('合同', 'CONTRACT'); var contractList = document.createElement('dl'); contractList.className = 'lookup-data-list'; addDetail(contractList, '合同编号', order.contract.number || '租赁合同'); addDetail(contractList, '合同状态', order.contract.statusLabel); if (order.contract.signedAt) addDetail(contractList, '签署时间', order.contract.signedAt); contract.appendChild(contractList); if (order.contract.url) { var link = document.createElement('a'); link.className = 'btn btn-primary'; link.href = order.contract.url; link.target = '_blank'; link.rel = 'noopener'; link.textContent = order.contract.status === 'pending_sign' ? '打开合同并签署' : '查看 / 下载合同'; contract.appendChild(link); } paymentContract.appendChild(contract); } else if (['approved', 'awaiting_signature', 'pending_payment', 'pending_pickup', 'paid', 'active', 'completed'].indexOf(order.status) !== -1) { var pendingContract = section('合同', 'CONTRACT'); var pendingNote = document.createElement('p'); pendingNote.className = 'section-note contract-empty-note'; pendingNote.textContent = '订单已审核通过，合同签署链接正在生成，请稍后刷新订单。'; pendingContract.appendChild(pendingNote); paymentContract.appendChild(pendingContract); }
    grid.appendChild(paymentContract);
